  ...
  <div class="">
    ...
    <div class="row">
      <div class="col-md-12 col-sm-12 col-xs-12" style="margin-bottom: 1em;margin-top: 5em;">
        <span class="label label-info" data-toggle="modal" data-target=".entry-modal">Create New District</span>
      </div>
    </div>
    <div class="clearfix"></div>
    <div class="row">
          ...
          <div class="x_content">
            <div class="table-responsive">
              <table id="order-table" class="table table-striped jambo_table bulk_action">
                <thead>
                  <tr class="headings">
                    <th class="column-title">District Code</th>
                    <th class="column-title">District Name</th>
                    <th class="column-title">Operational Code</th>
                ...
                <tbody class="tracker_tbody">
                  ...
                  <tr class="post{{$district->id}}">
                    <td class=" ">{{ $district->code}}</td>
                    <td class=" ">{{ $district->name}}</td>
                    <td class=" ">{{ $district->opcode}}</td>
                    <td class="a-right a-right ">
                      <button data-id="{{$district->id}}" data-code="{{$district->code}}" data-name="{{$district->name}}" data-opcode="{{$district->opcode}}" type="button" class="btn btn-demo edit-btn" data-toggle="modal" data-target=".single-modal">
                    Edit
                      </button>
                    </td>
                    <td class="a-right a-right ">
                      <button data-id="{{$district->id}}" data-code="{{$district->code}}" data-name="{{$district->name}}" data-opcode="{{$district->opcode}}" type="button" class="btn btn-demo dlt-btn" data-toggle="modal" data-target=".alert-modal">
                    Delete
                      </button>
                    </td>
                  </tr>
                ...
                <div class="modal-dialog modal-sm">
                  <div class="modal-content">
                    ...
                    <div class="modal-body">
                      <form action="{{route('deleteDistrict')}}" method="POST" role="form" id="district-delete" class="form-horizontal form-label-left">
              ...
              <div class="modal-dialog modal-lg">
                <div class="modal-content">
                  <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel">District Form</h4>
                  </div>
                  <div class="modal-body">
                    <form action="{{route('addDistrict')}}" method="POST" role="form" id="ddistrict" class="form-horizontal form-label-left">
                      ...
                      <div class="form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12">District Name</label>
                        <div class="col-md-9 col-sm-9 col-xs-12">
                          <input type="text" id="name" name="name" class="form-control" placeholder="" required>
                        </div>
                      </div>
                      <div class="form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12">Operational Code</label>
                        <div class="col-md-9 col-sm-9 col-xs-12">
                          <select required="true" class="form-control" id="opcode" name="opcode">
                            <option value="">Select OpCode</option>
                 
                            <option value="{{ $opco }}">  </option>
                          </select>
                        </div>
                      </div>
                    </div>
            ...
            <div id="myModal" class="modal fade bs-example-modal-lg single-modal" tabindex="-1" role="dialog" aria-hidden="true">
              <div class="modal-dialog modal-lg">
                <div class="modal-content">
                  ...
                  <div class="modal-body">
                    <form action="{{route('editDistrict')}}" method="POST" role="form" id="district-update" class="form-horizontal form-label-left">
                      ...
                      <div class="form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12">Operational Code</label>
                        <div class="col-md-9 col-sm-9 col-xs-12">
                          <select class="form-control" id="opcode" name="opcode">
                            <option>Select OpCode</option>
                            <option value="{{ $opco }}">  </option>
                          </select>
                        </div>
                      </div>
  ...
  $.ajax({
    ...
    success:function(data)
    {
      ...
      $('#order-table').append("<tr class='post" + data.id + "'>"+
        ...
        data.code + "' data-name='" +
        data.name + "' data-opcode='" +
        data.opcode + "'>Edit</button></td>"+
...
$(document).on('click', '.edit-btn', function(e) {
  ...
  $.get("{{route('districtedit')}}",{id:id},function(data){
    ...
    $('#district-update').find('#opcode').val(data.opcode);
    $('#myModal').modal('show');
    console.log(data.id);
  })
});
